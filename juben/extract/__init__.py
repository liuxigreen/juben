...
class ContextExtractor:
    ...
    def build_scribe_prompt(self, context: dict, outline: Optional[ChapterOutline] = None) -> str:
        ...
        # === 系统角色 ===
        parts.append(
            "你是一个深谙抖音/红果爆款逻辑的顶级短剧编剧。你写的剧本台词密集（约每分钟400字、"
            "每秒1句）、情绪拉满、每15-30秒一个反转、钩子前置——观众永远在等下一句。"
            "你尊重观众：每句台词要么推进信息要么拉升情绪，绝不注水。"
            "基于以下Python注入的硬事实，撰写第"
        ...
        parts.append("8. 三人及以上对白格式要求：当场上同时存在3个及以上角色时，必须使用带主语的说话引导语（如「林小雨冷笑道：」「王建国沉声道：」），禁止使用代词引导（如「他说：」「她问：」），以避免Extracto代词消解失败。")
        parts.append("9. 主角内心OS是短剧的情绪引擎：每章允许1-3句内心独白（如「这一世，我定要她血债血偿」），放在打脸或复仇的关键节拍上。")
        parts.append("10. 台词密度：目标每分钟380-460字台词（约每秒1句、单句≤15字）；一场对手戏10-20个回合，每回合立场/信息/情绪至少变一个；全章对话占比40%-72%。")
...
### ❌ 错误3：信息倾倒式独白（注意：错的不是"对话多"，是"单人连续灌输"）
"'三年前...'沈渡的声音很轻。'太子在东宫设宴...席间有人中毒...'"
"'后来证人翻供。'沈渡说。'因为有人威胁了他的家人。而那个威胁我的人...'"

→ 问题：一个角色连续3句以上在单方面陈述背景信息，对方只当人肉提问机。观众不需要听汇报。
→ 正确：短剧对话就是你一句我一句的攻防——把信息拆碎塞进交锋里，每回合立场/信息/情绪至少变一个。对手要抢话、反咬、掀桌，不是"后来呢？"。
